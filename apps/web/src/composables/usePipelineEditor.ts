...
function defaultNode(type: PipelineNode['type']): PipelineNode {
  ...
  switch (type) {
    ...
    case 'prompt-wrapper': return {id, type: 'prompt-wrapper', artifactPrefix: id, config: {tagName: 'instructions', instructionText: '', includeInputArtifact: true, inputPlacement: 'after-instructions'}};
...
export function usePipelineEditor(initialDef: PipelineDefinition) {
  ...
  function sync() {
    ...
    // Auto-set outputRef to the last node's primary output
  ...
  function moveNode(nodeId: string, direction: 'up' | 'down') {
    ...
    // Don't move input node or swap with it
  ...
  function updateNode(nodeId: string, patch: Record<string, unknown>) {
    def.value = {
      ...def.value,
      nodes: def.value.nodes.map((n) => n.id === nodeId ? deepMerge(n as Record<string, unknown>, patch) as unknown as PipelineNode : n),
    };
    sync();
  }

  return {def, finalArtifactKey, addNode, removeNode, moveNode, updateNode};
}
