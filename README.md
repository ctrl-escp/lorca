...
- **Step-chain only:** pipelines and Capsules use ordered `steps[]` as the canonical body.
...
| Package | Role | Status |
| --- | --- | --- |
| `@lorca/core` | Domain types: `PipelineDefinition`, `CapsuleDefinition`, artifacts, traces, export file shapes | ✅ Implemented |
| `@lorca/prompt` | `buildUserPromptArtifacts`, `renderTemplate`, XML/tag helpers | ✅ Implemented |
| `@lorca/endpoints` | `testBrowserAccess`, `listModels`, `executeModelCall`; `ollamaAdapter` + bucket heuristics | ✅ Implemented |
| `@lorca/pipeline` | `validatePipeline`, `executeStepChain`, artifact key helpers | ✅ Implemented |
...
A **Capsule** is a reusable step-chain subgraph with a public interface (input ports, output ports, parameters, model slots). Capsules persist as `steps[]` at schema version 2.
...
An **artifact** is an immutable named value produced during a run (e.g. `user_prompt.raw`, `criteria.extracted_json`). Keys are namespaced by step `outputNamespace` to avoid collisions when the same Capsule appears twice. Capsule **internal** artifacts live under `${prefix}.internal.*` and are visible in the trace but not promoted to the parent namespace unless bound as public outputs.
...
| `presentation` | Renders static/template text against artifacts (`{{artifact.key}}`) |
| `capsule-instance` | Runs a Capsule subgraph inside the parent pipeline |
| `loop-group` | Repeats an inner step chain until an exit condition or iteration cap |

The executor runs enabled steps in chain order.
